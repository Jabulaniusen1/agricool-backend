...
def get_total_in_cooling_fees(
    ...
    **kargs
    ...
    if cup_crop.pricing.pricing_type == "FIXED":
        return quantitize_float(metric_multiplier * cup_crop.pricing.fixed_rate, crate.currency)
    elif cup_crop.pricing.pricing_type == "PERIODICITY":
        return quantitize_float(metric_multiplier * storage_duration_days * cup_crop.pricing.daily_rate, crate.currency)

    return 0


@transaction.atomic
def create_partial_checkout(crate, weight_in_kg, checkout=None, cooling_fees=None, compute_dependencies=True):
    ...
    if cooling_fees is None:
        cooling_fees = get_total_due_in_cooling_fees(crate) * percentage
...
def get_storage_duration_days(crate, checkout_date=None, checkin_date=None):
    ...
    if crate.produce and crate.produce.checkin:
        ...
        if not checkin_date and crate.produce.checkin.movement:
            ...
            checkin_date = movement.date.replace(
                tzinfo=None
            ).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    is_same_day = checkin_date > checkout_date or checkin_date.date() == checkout_date.date()
    is_marketplace_originated_crate = str(crate.produce and crate.produce.checkin.movement.initiated_for) == str(Movement.InitiatedFor.MARKERPLACE_ORDER)
    ...
    return (
        0 if is_marketplace_originated_crate and is_same_day
...
def get_total_due_in_cooling_fees_per_kg(crate, **kargs):
    total_due_in_cooling_fees_per_kg = get_total_due_in_cooling_fees(crate, **kargs) / max(1, crate.weight)
