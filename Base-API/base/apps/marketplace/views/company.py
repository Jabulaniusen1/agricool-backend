...
from base.apps.marketplace.models import (CompanyDeliveryContact, Order,
                                          OrderCrateItem, PaystackAccount)
from base.apps.marketplace.serializers.common import (
    CompanyDeliveryContactSerializer, PaystackAccountSerializer)
...
from base.apps.marketplace.serializers.company_responses import \
...
class CompanyOrdersViewSet(ViewSet):
    ...
    def list(self, request):
        ...
        orders = Order.objects.prefetch_related(
            Prefetch('items', queryset=OrderCrateItem.objects.filter(
...
class CompanyDeliveryContactsViewSet(ViewSet):
    ...
    def get_details(self, company, company_delivery_contact_id=None):
        return get_object_or_404(CompanyDeliveryContact,
    ...
    def create(self, request):
        ...
        input = CompanyCreateDeliveryContactRequestSerializer(data=request.data)
        ...
        delivery_contact = CompanyDeliveryContact.objects.create(
            ...
            delivery_company_name=input.validated_data['delivery_company_name'],
            contact_name=input.validated_data['contact_name'],
            phone=input.validated_data['phone'],
        )
    ...
    def retrieve(self, request, company_delivery_contact_id=None):
        ...
        company_delivery_contacts = self.get_details(company, company_delivery_contact_id)
...
class CompanySetupViewSet(ViewSet):
    ...
    def eligibility_check(self, request):
        ...
        input = CompanySetupEligibilityCheckRequestSerializer(data=request.data)
        input.is_valid(raise_exception=True)  # Validate and raise exceptions if invalid

        companies_elligibility_map = {}
        users_elligibility_map = {}

        company_ids = input.validated_data.get('company_ids', [])
        user_ids = input.validated_data.get('user_ids', [])
        ...
        for company_id in PaystackAccount.objects.filter(owned_on_behalf_of_company_id__in=company_ids, is_default_account=True).values_list('owned_on_behalf_of_company_id', flat=True).iterator():
            companies_elligibility_map[str(company_id)] = True
        for user_id in PaystackAccount.objects.filter(owned_by_user_id__in=user_ids, owned_on_behalf_of_company_id__isnull=True, is_default_account=True).values_list('owned_by_user_id', flat=True).iterator():
            users_elligibility_map[str(user_id)] = True

        return Response({ "companies": companies_elligibility_map, "users": users_elligibility_map }, status=HTTP_200_OK)
    ...
    def get_users_bank_account_details(self, request, company_id=None):
        ...
        if not user_id:
            return Response({"error": "Expecting user_id in query params"}, status=HTTP_400_BAD_REQUEST)

        user = get_object_or_404(User, id=user_id)

        paystack_account = PaystackAccount.get_user_default_account(user.id)
        if not paystack_account:
            return Response({"error": "No default paystack account found for user"}, status=HTTP_400_BAD_REQUEST)

        serializer = PaystackAccountSerializer(paystack_account)
        return Response(serializer.data, status=HTTP_200_OK)

    @action(methods=['POST'], url_path='users-first-paystack-bank-account', detail=False)
    @transaction.atomic
    def setup_users_first_paystack_bank_account(self, request):
        """ POST /marketplace/company/setup/users-first-paystack-bank-account/ - Setup users first paystack bank account """

        # identify the company tied with this operator
        # company = get_company(request)

        # Validate data with the serializer
        input = CompanySetupUsersFirstPaystackBankAccountRequestSerializer(data=request.data)
        input.is_valid(raise_exception=True)  # Validate and raise exceptions if invalid

        # Get target user and validate that the operator can access it
        user = get_object_or_404(User,
            id=input.validated_data['owned_by_user_id'],
            own_paystack_accounts__isnull=True,
            # TODO: check if the access to the company is valid
        )
        ...
        try:
            paystack_account = PaystackAccount.create(
                created_by_user=request.user,
                owned_by_user=user,
                account_type=input.validated_data['account_type'],
                bank_code=input.validated_data['bank_code'],
                country_code=input.validated_data['country_code'],
                account_number=input.validated_data['account_number'],
                account_name=input.validated_data['account_name'],
            )
