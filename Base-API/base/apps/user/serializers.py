...
from base.apps.storage.models import (CoolingUnit, CoolingUnitCrop, Crop,
                                      Pricing, Produce)
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = (
            'id', 'username', 'phone', 'email', 'gender', 'password',
            'first_name', 'last_name', 'last_login', 'language',
            'is_email_public', 'is_phone_public'
        )
        extra_kwargs = {
    ...
    def to_representation(self, instance):
        ...
        if not instance.is_phone_public and not instance.is_email_public:
            if not user.is_authenticated:
                ...
                company_id = operator.company_id if operator else (employee.company_id if employee else None)
    ...
    def update(self, instance, validated_data):
        ...
        if (cooling_units != None):
            ...
            operator_cooling_units = CoolingUnit.objects.filter(operators=instance).values('id')
            ...
            for op_cu in operator_cooling_units:
                if op_cu['id'] not in cooling_units:
                    CoolingUnit.objects.get(id=op_cu['id']).operators.remove(instance.id)
    ...
    def update(self, instance, validated_data):
        ...
        if instance.country != validated_data['country']:
            ...
            crops = Crop.objects.filter(
                countryRelated__country=validated_data['country']
            ).exclude(name="Other").values_list('id', flat=True)
            if validated_data['country'] == "IN" or validated_data['country'] == "NG":
            ...
            for cu in cooling_units:
                ...
                CoolingUnitCrop.objects.filter(cooling_unit=cu).exclude(crop__name='Other').update(active=False)

                # If a crop in the request isn't already attached to this cooling unit, add it with the default price of the other crops
                first_unit_pricing = CoolingUnitCrop.objects.filter(cooling_unit=cu).first()
                ...
                for crop in crops:
                    ...
                    if cu_crop:
                        CoolingUnitCrop.objects.filter(crop_id=crop, cooling_unit=cu).update(active=True)
                    # only create a new entry if pricing_instance is not None and the crop is not already attached to the cooling unit
                        ...
                        CoolingUnitCrop.objects.create(crop_id=crop, cooling_unit=cu, pricing=new_pricing_instance,
                                                       active=True)
        else:
...
class OperatorSerializer(serializers.ModelSerializer):
    ...
    def get_cooling_units(self, instance):
        cooling_units = CoolingUnit.objects.filter(operators=instance.user, deleted=False).values_list('id', flat=True)
...
class FarmerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Farmer
        fields = ('id', 'user', 'birthday', 'parent_name', 'country', 'user_code', 'companies', 'cooling_units')
    ...
    def validate(self, data):
        ...
        if createUser:
            if Farmer.objects.filter(user__phone=user.get('phone', None), smartphone=True).exists():
                raise serializers.ValidationError({'user': [_('User with this phone number already exists.')]})
            return {
                'user': user,
        ...
        if not update_user:
            if 'cooling_unit_id' in validated_data:
                ...
                if instance.companies.filter(pk=company_id).exists():
                    ...
                    cooling_units = instance.cooling_units.filter(location__company_id=company_id)
                    for cu in cooling_units:
    ...
    def create(self, validated_data):
        ...
        serialized_user = UserSerializer(data=user, context={'request': self.context['request']})
        if not serialized_user.is_valid():
            raise serializers.ValidationError({'user': [_('Error creating the user')]})

        if createUser:
            try:
                farmer_instance = Farmer.objects.get(user__phone=user.get('phone', None))
                ...
                farmer_instance = Farmer.objects.create(**validated_data, user=user_instance)
            ...
            if unique_user_code is None:
                unique_user_code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
...
class ServiceProviderRegistrationSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if Company.objects.filter(name=company.get('name', None)).exists():
            raise serializers.ValidationError({'company': [_('Company with that name already exists.')]})
    ...
    def create(self, validated_data):
        ...
        serialized_op = UserSerializer(data=default_op, context={'request': self.context['request']})
        ...
        serialized_farmer = UserSerializer(data=unknown_farmer, context={'request': self.context['request']})
        ...
        company_instance = Company.objects.create(name=company["name"], country=company["country"],
                                                  currency=company["currency"], bank_account= bank_instance_created,
        ...
        instance = ServiceProvider.objects.create(**validated_data, user=user_instance, company=company_instance)
        return instance
...
class ServiceProviderRegistrationWithInvitationSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        invitation = InvitationUser.objects.filter(
            code=code
        ).last()
        ...
        if User.objects.filter(Q(phone=str(invitation.phone)) | Q(email=email
                                                                  )).exists():
            raise serializers.ValidationError({'user': [_('User already exists.')]})
    ...
    def create(self, validated_data):
        ...
        instance = ServiceProvider.objects.create(user=user_instance, company=company_instance)
        return instance
...
class OperatorRegistrationWithInvitationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        instance = Operator.objects.create(**validated_data, user=user_instance, company=company_instance)
        for cu in cooling_units:
...
class ServiceProviderLoginSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        data['user'] = {"id": sp.user.id, "first_name": sp.user.first_name, "last_name": sp.user.last_name,
                        "email": sp.user.email, "gender": sp.user.gender, "phone": str(sp.user.phone),
...
class OperatorLoginSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        # Add extra responses here
        data['user'] = {"id": op.user.id, "first_name": op.user.first_name, "last_name": op.user.last_name,
                        "gender": op.user.gender, "phone": str(op.user.phone), "last_login": str(op.user.last_login)}
        data['role'] = 'Operator'
        data['company'] = company_serializer.data
        return data
...
class FarmerLoginSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        # Add extra responses here
        data['user'] = {"id": f.user.id, "first_name": f.user.first_name, "last_name": f.user.last_name,
                        "gender": f.user.gender, "phone": str(f.user.phone), "last_login": str(f.user.last_login)}
...
class FarmerSurveySerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        farmer_survey_instance = FarmerSurvey.objects.create(**validated_data,
        ...
        for crop_item in commodities:
            if (crop_item['crop_id'] != ''):
                FarmerSurveyCommodity.objects.create(
                    farmer_survey_id=farmer_survey_instance.id, crop_id=crop_item['crop_id'],
                    average_price=crop_item['average_price'], unit=crop_item['unit'],
                    ...
                    quantity_below_market_price=crop_item['quantity_below_market_price'],
                    average_season_in_months=crop_item['average_season_in_months'], currency=crop_item['currency'],
                    date_filled_in=datetime.now().astimezone(), date_last_modified=datetime.now().astimezone(),
                )
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    def get_market_listing(self, instance):
        try:
            if instance.event_type == Notification.NotificationType.LISTING_PRICE_UPDATED:
                market_listing_price = MarketListedCratePrice.objects.get(id=instance.specific_id)
    ...
    def get_cooling_unit_name(self, instance):
        try:
            ...
            if instance.event_type == Notification.NotificationType.LISTING_PRICE_UPDATED:
                cu = CoolingUnit.objects.filter(crate_cooling_unit__market_listed_crates__prices__id=instance.specific_id).first().name
    ...
    def get_crates(self, instance):
        try:
            ...
            if instance.event_type == 'TIME_TO_PICKUP' or instance.event_type == 'FARMER_SURVEY':
                produce = Produce.objects.get(id=instance.specific_id)

            elif instance.event_type == 'MARKET_SURVEY':
                produce = Produce.objects.filter(crates__partial_checkouts__checkout_id=instance.specific_id).first()

            elif instance.event_type == 'CHECKOUT_EDITED':
                produce = Produce.objects.filter(checkin_id=instance.specific_id).first()

            elif instance.event_type == 'ORDER_REQUIRES_MOVEMENT':
                produce = Produce.objects.filter(checkin__movement_id=instance.specific_id).first()

            elif instance.event_type == Notification.NotificationType.LISTING_PRICE_UPDATED:
                produce = Produce.objects.filter(crates__market_listed_crates__prices__id=instance.specific_id).first()
    ...
    def get_movement_code(self, instance):
        try:
            if instance.event_type == 'MARKET_SURVEY':
                movement_code = Checkout.objects.get(id=instance.specific_id).movement.code
                return movement_code
            elif instance.event_type == 'TIME_TO_PICKUP' or instance.event_type == 'FARMER_SURVEY':
                movement_code = Produce.objects.get(id=instance.specific_id).checkin.movement.code
                return movement_code
            elif instance.event_type == 'CHECKIN_EDITED':
                movement_code = Checkin.objects.get(id=instance.specific_id).movement.code
                return movement_code
            elif instance.event_type == Notification.NotificationType.LISTING_PRICE_UPDATED:
                movement_code = Checkin.objects.filter(produces__crates__market_listed_crates__prices__id=instance.specific_id).first().movement.code
                return movement_code
        except:
            pass
